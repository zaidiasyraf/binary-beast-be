...
public interface CustomerRelationshipRepository extends JpaRepository<CustomerRelationship, Long> {
    ...
    @Query(value = "SELECT cr.*, 1 - (cr.embedding <=> CAST(:embedding AS vector(1536))) AS similarity " +
                   "FROM customer_relationships cr " +
                   "WHERE cr.embedding IS NOT NULL " +
                   "ORDER BY cr.embedding <=> CAST(:embedding AS vector(1536)) " +
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE customer_relationships SET embedding = CAST(:embedding AS vector(1536)) WHERE relationship_id = :relationshipId", nativeQuery = true)
    void updateRelationshipEmbedding(@Param("relationshipId") Long relationshipId, @Param("embedding") String embedding);
}
